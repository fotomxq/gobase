...
//return error 错误信息
func UploadFile(headerParams ServerHeaderParams,formName string,targetSrc string,maxSize int64,filterType []string,isRename bool) (FileUploadType,error) {
	//初始化
	res := FileUploadType{}
	//获取文件
	formFile,header,err := headerParams.R.FormFile(formName)
	if err != nil{
		log.SendError(headerParams.IP,headerParams.UserInfo.Username,"ServerType.UploadFile.GetFile",err)
		return res,err
	}
	defer formFile.Close()
	//判断文件尺寸
	if header.Size > maxSize && maxSize > 0{
		log.SendError(headerParams.IP,headerParams.UserInfo.Username,"ServerType.UploadFile.FileSize",errors.New("upload file size too lager."))
		return res,err
	}
	...
	if len(filterType) > 0{
		...
		if isOK == false{
			log.SendError(headerParams.IP,headerParams.UserInfo.Username,"ServerType.UploadFile.FileType",errors.New("upload file type is ban."))
			return res,err
		}
	}
	//创建时间
	...
	if err != nil{
		log.SendError(headerParams.IP,headerParams.UserInfo.Username,"ServerType.UploadFile.Sha1ReadFile",err)
		return res,err
	}
	dataByte := buf[:n]
	hash := sha1.New()
	if _, err := io.WriteString(hash, string(dataByte)); err!= nil {
		log.SendError(headerParams.IP,headerParams.UserInfo.Username,"ServerType.UploadFile.GetSha1",err)
		return res,err
	}
	...
	if err != nil{
		log.SendError(headerParams.IP,headerParams.UserInfo.Username,"ServerType.UploadFile.SaveFile",err)
		return res,err
	}
	//返回
	return res,nil
